...
void *best_fit_allocator(unsigned int size)
{
	...
	size_t minSz = UINT64_MAX;
	struct mem_region *current_region = free_regions;
	struct mem_region *smallest_region = NULL;

	do
	{
		if (size <= current_region->size && minSz > current_region->size)
		{
			smallest_region = current_region;
			minSz = current_region->size;
		}
	...
	if (smallest_region != NULL)
	{
		struct mem_region *tmp =
			(struct mem_region *)malloc(sizeof(struct mem_region));
		current_region = smallest_region;
...
void *worst_fit_allocator(unsigned int size)
{
	...
	size_t maxSz = 0;
	struct mem_region *current_region = free_regions;
	struct mem_region *largest_region = NULL;

	do
	{
		if (size <= current_region->size && maxSz < current_region->size)
		{
			largest_region = current_region;
			maxSz = current_region->size;
		}
